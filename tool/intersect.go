...
//IntersectJson 求两个json 的交集
func IntersectJson(jsonStr1, jsonStr2 string) string {
	...
	if jsonStr2[:1] == "{" { //map类型
		...
		if SortedMap {
			...
			result := intersectSorttedMap(obj1, obj2) //调用递归合并方法
			//将结果转回字符串返回
			...
			result := intersectMap(obj1, obj2) //调用递归合并方法
			//将结果转回字符串返回
			jsonBytes, err := json.Marshal(result)
			checkErr(err)
			return string(jsonBytes)
	...
	if jsonStr2[:1] == "[" { //list类型
		...
		result := intersectList(obj1, obj2) //调用递归合并方法
		//将结果转回字符串返回
		jsonBytes, err := json.Marshal(result)
		checkErr(err)
		return string(jsonBytes)
...
//intersectMap map递归合并操作
func intersectSorttedMap(map1, map2 *orderedmap.OrderedMap) *orderedmap.OrderedMap {
	resultMap := orderedmap.New()    //先创建空结果
	allKeys:=intersect(map1.Keys(),map2.Keys())
	for _,k:=range allKeys{
		...
		if exist1 && !exist2 {
			...
			case []interface{}: //list类型调用list的递归合并操作，并赋值给结果
				resultMap.Set(k, intersectList(v1.([]interface{}), v2.([]interface{})))
			case map[string]interface{}: //map类型调用map递归合并操作，并赋值给结果
				...
				//resultMap[k2] = intersectSorttedMap(v1.(map[string]interface{}), v2.(map[string]interface{}))
			case orderedmap.OrderedMap:
				...
				resultMap.Set(k, intersectSorttedMap(&v11, &v22))
			}
		}
	}
...
//intersectMap map递归合并操作,map1 新值，map2原始值
func intersectMap(map1, map2 map[string]interface{}) map[string]interface{} {
	resultMap := make(map[string]interface{}) //先创建空结果

	allKeys:=intersect(getKeys(map1),getKeys(map2))
	for _,k:=range allKeys{
		...
		if exist1 && !exist2{
			...
			case []interface{}: //list类型调用list的递归合并操作，并赋值给结果
				...
				resultMap[k] = intersectList(v1.([]interface{}), v2.([]interface{}))
			case map[string]interface{}: //map类型调用map递归合并操作，并赋值给结果
				resultMap[k] = intersectMap(v1.(map[string]interface{}), v2.(map[string]interface{}))
			}
		}
	}
	return resultMap //返回最终合并结果对象
...
//intersectList list递归合并操作
func intersectList(list1, list2 []interface{}) []interface{} {
	...
	for index, item2 = range list2 { //提取list2的索引以及内容
		...
		case []interface{}: //list类型调用list的递归合并操作，并赋值给结果
			resultList[index] = intersectList(list1[index].([]interface{}), item2.([]interface{}))
		case map[string]interface{}: //map类型调用map递归合并操作，并赋值给结果
			if SortedMap {
				fmt.Println("\033[31;1m list type fatal error initEnv.SortedMap \033[0m")
			}
			resultList[index] = intersectMap(list1[index].(map[string]interface{}), item2.(map[string]interface{}))
		case orderedmap.OrderedMap:
			...
			resultList[index] = intersectSorttedMap(&v11, &v22)
		}
	}
	...
	if len(list2)> len(list1) { //list2长度大于list1直接merge
		...
		//resultList = append(resultList, list1[index:]...)
	}
