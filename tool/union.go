...
//UnionJSON 合并两个json，用 jsonStr1 的值覆盖 jsonStr2 的值，并 保留 jsonStr2中不存在的内容。注意剔除json前后的空字符
func UnionJSON(jsonStr1, jsonStr2 string) string {
	//先看看两个json类型一样不，根据第一个字符判断
	if jsonStr1[:1] != jsonStr2[:1] {
		return "{ \"json1\":"+jsonStr1+",\"json2\":"+jsonStr2+"}" //合并
	}
	var err error
	if jsonStr2[:1] == "{" { //map类型
		...
		if SortedMap {
			...
			result := unionSorttedMap(obj1, obj2) //调用递归合并方法
			//将结果转回字符串返回
			...
			result := unionMap(obj1, obj2) //调用递归合并方法
			//将结果转回字符串返回
			jsonBytes, err := json.Marshal(result)
			checkErr(err)
			return string(jsonBytes)
	...
	if jsonStr2[:1] == "[" { //list类型
		...
		result := unionList(obj1, obj2) //调用递归合并方法
		//将结果转回字符串返回
		jsonBytes, err := json.Marshal(result)
		checkErr(err)
		return string(jsonBytes)
...
//unionMap map递归合并操作
func unionSorttedMap(map1, map2 *orderedmap.OrderedMap) *orderedmap.OrderedMap {
	...
	for _,k:=range allKeys{
		...
		if exist1 && !exist2 {
			...
			case []interface{}: //list类型调用list的递归合并操作，并赋值给结果
				resultMap.Set(k, unionList(v1.([]interface{}), v2.([]interface{})))
			case map[string]interface{}: //map类型调用map递归合并操作，并赋值给结果
				...
				//resultMap[k2] = unionSorttedMap(v1.(map[string]interface{}), v2.(map[string]interface{}))
			case orderedmap.OrderedMap:
				...
				resultMap.Set(k, unionSorttedMap(&v11, &v22))
			}
		}
	}
...
//unionMap map递归合并操作,map1 新值，map2原始值
func unionMap(map1, map2 map[string]interface{}) map[string]interface{} {
	resultMap := make(map[string]interface{}) //先创建空结果

	allKeys:=union(getKeys(map1),getKeys(map2))
	for _,k:=range allKeys{
		...
		if exist1 && !exist2{
		  resultMap[k]=v1
			...
			case []interface{}: //list类型调用list的递归合并操作，并赋值给结果
				...
				resultMap[k] = unionList(v1.([]interface{}), v2.([]interface{}))
			case map[string]interface{}: //map类型调用map递归合并操作，并赋值给结果
				resultMap[k] = unionMap(v1.(map[string]interface{}), v2.(map[string]interface{}))
			}
		}
	}
	return resultMap //返回最终合并结果对象
...
//unionList list递归合并操作
func unionList(list1, list2 []interface{}) []interface{} {
	...
	for index, item2 = range list2 { //提取list2的索引以及内容
		...
		case []interface{}: //list类型调用list的递归合并操作，并赋值给结果
			resultList[index] = unionList(list1[index].([]interface{}), item2.([]interface{}))
		case map[string]interface{}: //map类型调用map递归合并操作，并赋值给结果
			if SortedMap {
				fmt.Println("\033[31;1m list type fatal error initEnv.SortedMap \033[0m")
			}
			resultList[index] = unionMap(list1[index].(map[string]interface{}), item2.(map[string]interface{}))
		case orderedmap.OrderedMap:
			...
			resultList[index] = unionSorttedMap(&v11, &v22)
		}
	}
	fmt.Println(index, len(list1))
	if len(list2)> len(list1) { //list2长度大于list1直接merge
		resultList = append(resultList, list2[index+1:]...)
